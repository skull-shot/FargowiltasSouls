...
float4 PixelShaderFunction(VertexShaderOutput input) : COLOR0
{
	...
	float fadeMapBrightness = tex2D(uImage1, float2(frac(coords.x * 0.9 - globalTime * 4), coords.y)).r;
    
    float bloomOpacity = lerp(pow(abs(sin(coords.y * 3.141)), lerp(2, 6, coords.x)), 0.7, coords.x);
    
    if (coords.x < 0.85)
    {
        float mult = (1 - (coords.x / 0.85)) * pow((0.5 - abs(coords.y - 0.5)), 3.8);
    ...
    if (coords.x > 0.9)
        opacity *= pow(1 - ((coords.x - 0.9) / 0.1), 2);
    
    return color * opacity * pow(abs(bloomOpacity), 4) * lerp(0.8, 10, pow(fadeMapBrightness, 2));
}
