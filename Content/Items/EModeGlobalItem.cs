    ...
    public class EModeGlobalItem : GlobalItem
    {
        ...
        */
        ...
        public override void ModifyWeaponCrit(Item item, Player player, ref float crit)
        {
            switch (item.type)
            {
                case ItemID.TopazStaff:
                    if (EmodeItemBalance.HasEmodeChange(player, ItemID.TopazStaff).Any(s => s == "TopazStaff"))
                        crit += 6;
                    break;
                default:
                    break;
        ...
        public override float UseSpeedMultiplier(Item item, Player player)
        {
            ...
            if (item.type == ItemID.SapphireStaff && EmodeItemBalance.HasEmodeChange(player, item.type).Any(s => s == "SapphireStaff"))
            {
                return 1.5f;
            }
            if (item.type == ItemID.EmeraldStaff && EmodeItemBalance.HasEmodeChange(player, item.type).Any(s => s == "EmeraldStaff"))
            {
                return 0.75f;
            }
            return base.UseSpeedMultiplier(item, player);
        ...
        public override void ModifyShootStats(Item item, Player player, ref Vector2 position, ref Vector2 velocity, ref int type, ref int damage, ref float knockback)
        {
            ...
            {                    
                case ItemID.ChlorophyteSaber:
                    if (EmodeItemBalance.HasEmodeChange(player, item.type).Any(s => s == "ChlorophyteSaber"))
                    {
                        velocity *= 2f;
                    }
                    break;
                ...
                case ItemID.JackOLanternLauncher:
                    if (EmodeItemBalance.HasEmodeChange(player, item.type).Any(s => s == "JackOLanternLauncher"))
                    {
                        velocity *= 1.5f;
                    }
                    break;
                case ItemID.SapphireStaff:
                    if (EmodeItemBalance.HasEmodeChange(player, item.type).Any(s => s == "SapphireStaff"))
                    {
