...
{
    public class Vulture : EModeNPCBehaviour
    {
        ...
        public override bool SafePreAI(NPC npc)
        {
            ...
            if (SwoopTimer >= swoopStart)
            {
                ...
                if (SwoopTimer > startupEnd && SwoopTimer <= telegraphEnd) // telegraph, slowdown
                {
                    ...
                    {
                        FargoSoulsUtil.DustRing(npc.Center, 32, DustID.Sand, 10f);
                    }
                    npc.velocity *= 0.85f;
                    npc.direction = npc.spriteDirection = npc.HorizontalDirectionTo(player.Center).NonZeroSign();
                    SwoopTimer++;
                ...
                if (SwoopTimer > telegraphEnd && SwoopTimer <= swoopEnd) // swoop
                {
                    ...
                    if (SwoopTimer == feather1 || SwoopTimer == feather2)
                    {
                        ...
                        {
                            Projectile.NewProjectile(npc.GetSource_FromAI(), npc.Center, -Vector2.UnitY.RotatedByRandom(MathHelper.PiOver2 * 0.3f) * 5, ModContent.ProjectileType<VultureFeather>(), FargoSoulsUtil.ScaledProjectileDamage(npc.defDamage), 1f);
                        }
                    }
