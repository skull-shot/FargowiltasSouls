...
namespace FargowiltasSouls.Content.Projectiles.Weapons.SwarmDrops
{
    ...
    public class SlimeSlingingSlasherProj : ModProjectile
    {
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            Projectile.NewProjectile(Terraria.Entity.InheritSource(Projectile), Main.rand.NextVector2FromRectangle(target.Hitbox), Vector2.Zero, ModContent.ProjectileType<Slimesplosion>(), 0, 1f, Projectile.owner);
            ...
            void SlimeProjs(Player player)
            {
                if (Main.myPlayer == Projectile.owner)
                {
                    for (int i = -7; i < 7; i++)
                    {
                        ...
                        Projectile.NewProjectile(Terraria.Entity.InheritSource(Projectile), player.RotatedRelativePoint(player.MountedCenter, true) + Vector2.Normalize(vel) * Projectile.height * 1f, vel, ModContent.ProjectileType<SlimeBallHoming>(), Projectile.damage / 2, Projectile.knockBack, player.whoAmI);
                    }
                }
                ProjectileCheck = 1;
            }
