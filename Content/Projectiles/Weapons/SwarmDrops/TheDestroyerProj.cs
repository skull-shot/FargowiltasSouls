                ...
                for (int i = 0; i < Main.maxProjectiles; i++)
                {
                    ...
                    if (p.Alive() && p.minion && p.owner == Projectile.owner && Cooldowns[p.whoAmI] <= 0 && Projectile.Colliding(Projectile.Hitbox, p.Hitbox))
                    {
                        ...
                        if (target != null)
                        {
                            ...
                            {
                                int p2 = Projectile.NewProjectile(Terraria.Entity.InheritSource(Projectile), p.Center, p.DirectionTo(target.Center) * 8, ModContent.ProjectileType<PrimeLaser>(), Projectile.originalDamage / 3, Projectile.knockBack, Projectile.owner, ai2: p.Distance(target.Center));
