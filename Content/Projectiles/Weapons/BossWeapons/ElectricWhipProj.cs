            ...
            if (LightningCounter < 4)
            {
                for (int i = 0; i < Main.maxProjectiles; i++)
                {
                    ...
                    if (p.Alive() && p.minion && p.owner == Projectile.owner && Cooldowns[p.whoAmI] <= 0 && p.Distance(target.Center) < 300)
                    {
                        ...
                        Projectile.NewProjectile(Terraria.Entity.InheritSource(Projectile), p.Center, p.DirectionTo(target.Center), ModContent.ProjectileType<ElectricWhipLightning>(), Projectile.originalDamage / 2, Projectile.knockBack, Projectile.owner, ai2: p.Distance(target.Center));
                        Cooldowns[p.whoAmI] = 45;
                        LightningCounter++;
                        if (LightningCounter >= 4)
                            return;
                    }
